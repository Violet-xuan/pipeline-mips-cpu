...
| 地址范围（字节） | 功能 |
|------------------|------|
| 0x00000000 ~ 0x00003FFF | 数据存储器 RAM（16KB = 4096 字，扩大自 PDF 的 2KB） |
...
- **DMEM**：**4096 字 RAM（16KB）**，同步写、异步读，用 `$readmemh` 预置测试数据（掉电重载即恢复，符合"现场改数据只换 hex"）。
  从 512 字扩大：DFS 记忆表 O((m+n-1)·m²) 字，512 字仅够 m,n≤~5；4096 字可容纳约 ≤11×11。
  `$sp` 初始化指向 RAM 高端（0x3FF4）。`sbrk` 分配区由汇编改写为"bump 指针"固定地址（grid 原地，memo 紧跟其后）。
- **MemBus**（MEM 阶段地址译码器/"外设控制器"）：`addr[30]` 判定 `0x4xxxxxxx` 走外设，否则走 DMEM（`addr[13:2]` 索引）。
- **digi 寄存器语义**（PDF 第 11–12 页固定）：
  `digi[11:8]` = AN3..AN0（位选，高电平选中），`digi[7]` = dp，`digi[6:0]` = g..a（高电平点亮）。
  软件方式译码（`seg7` 子程序，16 路分支）+ 软件延时扫描（每位轮显）。**不使用硬件译码器。**

### 约束：数据 RAM 容量
传纸条改写版的 grid + 记忆表 + 栈须放进 4096 字。realistic 现场数据较小（PDF 示例 3×3），
约 ≤11×11 可容纳；50×50（理论课 100% 数据）需 ~1MB，超出 BRAM，硬件版不覆盖。
更大网格可继续扩大 DMEM（XC7A35T BRAM ~225KB）。
